...
> **DDM is a cross-check column, not a third routing bucket.** `ddmPerShare` runs a two-stage dividend discount model — same growth/discount assumptions as the DCF (shares `presentValueOfGrowingFlow`), built on `annualCashDividendsPaid`/`quarterlyCashDividendsPaid` (already in `FIELDS`, no new Yahoo call) instead of free cash flow. It runs for every company regardless of `isFinancial()` — unlike the primary model, valuing what a company pays out doesn't depend on what kind of business it is — and returns `null` (dashed on the page) for the roughly half of the universe with no reliable dividend. Converted through the same `fxRate` as the primary model. The gap between DCF and DDM is deliberate signal: a company that generates more cash than it distributes (common among IDX's controlling-family firms) shows a DDM well below its main figure.
